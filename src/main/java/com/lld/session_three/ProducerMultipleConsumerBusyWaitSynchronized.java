...
public class ProducerMultipleConsumerBusyWaitSynchronized {
        ...
        for (int i = 0; i < numConsumers; i++) {
            ...
            consumers[i] = new Thread(() -> {
                while (true) {
                    ...
                    // C1 C2 C3 come out of the wait here

                    String orderName;
                    synchronized (orders) {  // Need synchronization here to avoid multiple consumers getting same order
                        if (orders.size() == 0) continue; // Double-check after getting lock
                        orderName = orders.poll();
                    }
